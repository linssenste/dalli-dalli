...
out skel qt;
    ...
    for (const element of shuffleArray(overpassData.elements).sort(() => Math.random()-0.5)) {
        if (counter>50) break;
        const googleStreetViewUrl=`https://maps.googleapis.com/maps/api/streetview?size=600x300&location=${element.lat},${element.lon}&return_error_code=true&pitch=10&heading=${Math.floor(Math.random()*360)}&key=${googleApiKey}`;
        try {
